...
class DiffDriveController(Node):
    ...
    def set_wheel_velocities(self, left_wheel_velocity, right_wheel_velocity):
        ...
        if abs(left_wheel_velocity) < 0.001 and abs(right_wheel_velocity) < 0.001:
            # Stop
            self.send_command("CMD:STOP")
            self.get_logger().info("Sending STOP command")
        elif left_wheel_velocity > 0 and right_wheel_velocity > 0 and abs(left_wheel_velocity - right_wheel_velocity) < 0.001:
            # Forward
            self.send_command("CMD:FORWARD")
            self.get_logger().info("Sending FORWARD command")
        elif left_wheel_velocity < 0 and right_wheel_velocity < 0 and abs(left_wheel_velocity - right_wheel_velocity) < 0.001:
            # Backward
            self.send_command("CMD:BACKWARD")
            self.get_logger().info("Sending BACKWARD command")
        elif left_wheel_velocity < 0 and right_wheel_velocity > 0:
            # Left turn
            ...
            # For differential speeds, we'll still use FORWARD or BACKWARD
            # based on the average direction
            avg_velocity = (left_wheel_velocity + right_wheel_velocity) / 2.0
            if avg_velocity > 0:
                self.send_command("CMD:FORWARD")
                self.get_logger().info("Sending FORWARD command (differential)")
            else:
                self.send_command("CMD:BACKWARD")
                self.get_logger().info("Sending BACKWARD command (differential)")
